...
As it is shown below, the new algorithm works very well in these toy examples.
However, it still needs to be tested in actual noisy conditions with more complex examples.
Moreover, as can be seen in the circle example, the algorithm can end up reducing some edges to zero even though it does not add much to the score, so it could be interesting to look into adding to the metric something that encourages keeping the relative lengths of the edges. 
...
![Circle.](./2026_04_06/Edge_matching-Circle.gif){group="examples-matching" #fig-examples-matching_circle}
...
Meeting did not happen as it was Easter Monday.
