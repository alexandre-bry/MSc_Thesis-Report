...
#let title-slide-centered(
  ...
  let body = {
    ...
    if logos != none {
      if type(logos) != array {
        logos = (logos,) // Wrap single logo in list for uniform handling
      }
      if logos.len() > 2 {
        panic("Too many logos provided. Maximum is 2.")
      }
      for logo in logos {
        logo
      }
...
/// Outline slide with elegant styling
#let outline-slide(
  config: (:),
  title: none,
  outline-content: outline(title: none, indent: 1em, depth: 1),
  colors: none,
) = touying-slide-wrapper(self => {
  let c = if colors != none { colors } else {
    self.store.at("theme-colors", default: get-theme-colors())
  ...
  // Body content (outline items)
  touying-slide(self: self, config: config)[
    #v(1fr)
    #set text(size: size-subtitle, fill: c.text-primary, weight: "medium")
    #outline-content
    #v(1fr)
  ]
})
