...
#let bullet-circle(size: 6pt, color: accent-secondary) = {
  box(circle(radius: size / 2, fill: color), inset: (y: size / 2))
}

// Level 2: Filled square
#let bullet-square(size: 5pt, color: accent-secondary) = {
  box(rect(width: size, height: size, fill: color), inset: (y: size / 2))
}

// Level 3: Dash/line
#let bullet-dash(size: 8pt, color: accent-secondary) = {
  let height = 2pt
  box(rect(width: size, height: height, fill: color, radius: 1pt), inset: (y: height / 2))
}
